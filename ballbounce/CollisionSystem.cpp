...
bool CollisionSystem::collision(Entity * e1, Entity * e2, glm::vec3 & impulseOUT, glm::vec3 & displacementOUT)
{
	if (e1->getType() == 1) {
		...
		glm::vec3 vecb = b->getPosition() - b->getPosition();
		...
		glm::vec3 impulsea = avecx * (a->getMass() * b->getMass()) / (a->getMass() + b->getMass()) + bvecx * (2 * b->getMass()) / (a->getMass() + b->getMass()) + avecy;
		glm::vec3 impulseb = avecx * (2 * a->getMass()) / (a->getMass() + b->getMass()) + bvecx * (b->getMass() - a->getMass()) / (a->getMass() + b->getMass()) + bvecy;

		//set velocity of spheres here

		return sphereSphereCollision(a, b, displacementOUT);
	}
	if (e1->getType() == 0) {
